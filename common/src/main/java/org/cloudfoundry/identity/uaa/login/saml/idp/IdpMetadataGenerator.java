...
/**
* The class is responsible for generation of identity provider metadata describing the identity provider in
...
public class IdpMetadataGenerator {
   ...
   public EntityDescriptor generateMetadata() {
       ...
       IDPSSODescriptor ssoDescriptor = buildIDPSSODescriptor(entityBaseURL, entityAlias, wantAuthnRequestSigned, includedNameID);
   ...
   protected IDPSSODescriptor buildIDPSSODescriptor(String entityBaseURL, String entityAlias, boolean wantAuthnRequestSigned, Collection<String> includedNameID) {
       ...
       for (String binding : bindingsSSO) {
           if (binding.equals(SAMLConstants.SAML2_ARTIFACT_BINDING_URI)) {
               idpDescriptor.getSingleSignOnServices().add(getSingleSignOnService(entityBaseURL, entityAlias, getSAMLWebSSOProcessingFilterPath(), SAMLConstants.SAML2_ARTIFACT_BINDING_URI));
           }
           if (binding.equals(SAMLConstants.SAML2_POST_BINDING_URI)) {
               idpDescriptor.getSingleSignOnServices().add(getSingleSignOnService(entityBaseURL, entityAlias, getSAMLWebSSOProcessingFilterPath(), SAMLConstants.SAML2_POST_BINDING_URI));
           }
           if (binding.equals(SAMLConstants.SAML2_PAOS_BINDING_URI)) {
               idpDescriptor.getSingleSignOnServices().add(getSingleSignOnService(entityBaseURL, entityAlias, getSAMLWebSSOProcessingFilterPath(), SAMLConstants.SAML2_PAOS_BINDING_URI));
           }
       }
...
//       }
       ...
       for (String binding : bindingsSLO) {
           if (binding.equals(SAMLConstants.SAML2_POST_BINDING_URI)) {
               idpDescriptor.getSingleLogoutServices().add(getSingleLogoutService(entityBaseURL, entityAlias, SAMLConstants.SAML2_POST_BINDING_URI));
           }
           if (binding.equals(SAMLConstants.SAML2_REDIRECT_BINDING_URI)) {
               idpDescriptor.getSingleLogoutServices().add(getSingleLogoutService(entityBaseURL, entityAlias, SAMLConstants.SAML2_REDIRECT_BINDING_URI));
           }
           if (binding.equals(SAMLConstants.SAML2_SOAP11_BINDING_URI)) {
               idpDescriptor.getSingleLogoutServices().add(getSingleLogoutService(entityBaseURL, entityAlias, SAMLConstants.SAML2_SOAP11_BINDING_URI));
           }
       }
       ...
       if (encryptionKey != null) {
           idpDescriptor.getKeyDescriptors().add(getKeyDescriptor(UsageType.ENCRYPTION, getServerKeyInfo(encryptionKey)));
       } else {
   ...
   protected SingleSignOnService getSingleSignOnService(String entityBaseURL, String entityAlias, String filterURL, String binding) {
   ...
   protected SingleSignOnService getHoKSingleSignOnService(String entityBaseURL, String entityAlias, String filterURL, String binding) {
       SingleSignOnService hokSso = getSingleSignOnService(entityBaseURL, entityAlias, filterURL, org.springframework.security.saml.SAMLConstants.SAML2_HOK_WEBSSO_PROFILE_URI);
