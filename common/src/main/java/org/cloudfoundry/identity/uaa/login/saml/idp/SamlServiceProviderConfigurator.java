...
import org.cloudfoundry.identity.uaa.login.saml.ConfigMetadataProvider;
import org.cloudfoundry.identity.uaa.login.saml.FixedHttpMetaDataProvider;
...
public class SamlServiceProviderConfigurator implements InitializingBean {
    private static Log logger = LogFactory.getLog(SamlServiceProviderConfigurator.class);
    private Map<SamlServiceProviderDefinition, ExtendedMetadataDelegate> serviceProviders = new HashMap<>();
    private List<SamlServiceProviderDefinition> toBeFetchedProviders = new LinkedList<>();
    ...
    public List<SamlServiceProviderDefinition> getServiceProviderDefinitions() {
        return Collections.unmodifiableList(new ArrayList<>(serviceProviders.keySet()));
    }

    public List<SamlServiceProviderDefinition> getServiceProviderDefinitionsForZone(IdentityZone zone) {
        List<SamlServiceProviderDefinition> result = new LinkedList<>();
        for (SamlServiceProviderDefinition def : getServiceProviderDefinitions()) {
    ...
    public List<SamlServiceProviderDefinition> getServiceProviderDefinitions(List<String> allowedSps,
            IdentityZone zone) {
        List<SamlServiceProviderDefinition> idpsInTheZone = getServiceProviderDefinitionsForZone(zone);
        if (allowedSps != null) {
            List<SamlServiceProviderDefinition> result = new LinkedList<>();
            for (SamlServiceProviderDefinition def : idpsInTheZone) {
    ...
    protected void parseServiceProviderDefinitions() {
        serviceProviders.clear();
        List<SamlServiceProviderDefinition> providerDefinitions = new LinkedList<>(toBeFetchedProviders);
        Set<String> uniqueAlias = new HashSet<>();
        for (SamlServiceProviderDefinition def : providerDefinitions) {
            String alias = getUniqueAlias(def);
            if (uniqueAlias.contains(alias)) {
                throw new IllegalStateException("Duplicate SP alias found:" + alias);
            }
            uniqueAlias.add(alias);
        }
        for (SamlServiceProviderDefinition def : providerDefinitions) {
            try {
                addSamlServiceProviderDefinition(def);
    ...
    public synchronized ExtendedMetadataDelegate removeServiceProviderDefinition(
    ...
    public void setServiceProviders(Map<String, Map<String, Object>> providers) {
        serviceProviders.clear();
        if (providers == null) {
            return;
        }
        for (@SuppressWarnings("rawtypes")
        Map.Entry entry : providers.entrySet()) {
            ...
            def.setSingleSignOnServiceIndex(assertionIndex == null ? 0 : assertionIndex);
    ...
    public void afterPropertiesSet() throws Exception {
        parseServiceProviderDefinitions();
    }
}
